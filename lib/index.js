...
const { untilShow, wait, click, fill, fillAndEnter, clear, scrollTo, uploadFile, datepicker, check, uncheck } = require('./interaction')
const { see, isChecked } = require('./query')
...
class Please {
    ...
    /** @param {string} label @param {string} selector */
    async check(label, selector)                        { return this._step(`Check "${label}"`, () => check(this.page, label, selector)) }
    /** @param {string} label @param {string} selector */
    async uncheck(label, selector)                       { return this._step(`Uncheck "${label}"`, () => uncheck(this.page, label, selector)) }
    ...
    /** @param {string} label @param {string} selector @param {boolean} [expected] @param {number} [time] */
    async isChecked(label, selector, expected, time)    { return this._step(`Is checked "${label}"`, () => isChecked(this.page, label, selector, expected, time)) }
