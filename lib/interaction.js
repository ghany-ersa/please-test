...
async function check(page, label, selector) {
    await untilShow(page, label, selector)
    await toLocator(page, selector).check()
}

async function uncheck(page, label, selector) {
    await untilShow(page, label, selector)
    await toLocator(page, selector).uncheck()
}

module.exports = { untilShow, wait, click, fill, fillAndEnter, clear, scrollTo, uploadFile, datepicker, check, uncheck }
